...
public class EventUserService {
    ...
    private void generateAndSendCode(String email, EventUser eventUser) {
        ...
        // 3. 인증 코드 정보를 데이터 베이스에 저장
        EmailVerification verification = EmailVerification.builder()
                .verificationCode(code) // 인증 코드
                .expiryDate(LocalDateTime.now().plusMinutes(5)) // 만료 시간
    ...
    public boolean isMatchCode(String email, String code) {
        ...
        if(eventUser!=null){
            // 인증코드가 있는지 탐색
            EmailVerification ev = emailVerificationRepository.findByEventUser(eventUser);

            if(
                    ev!=null && ev.getExpiryDate().isAfter(LocalDateTime.now())
                    && ev.getVerificationCode().equals(code)
            ){
                // 이메일 인증여부 true로 수정
        ...
        // 인증코드가 있고 만료시간이 지나지 않았고 코드번호가 일치한 경우
        return false;
    }
